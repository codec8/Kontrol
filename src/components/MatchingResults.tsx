  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-base font-semibold text-gray-800 dark:text-gray-200">Paycheck-to-Bill Matching</h2>
        <button
              ...
              <div className="space-y-1.5">
                {unpaidBills.map(allocation => (
                  <div
                    ...
                    className="p-2.5 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md"
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="text-xs text-red-600 dark:text-red-400">
                        Due: {format(allocation.expense.date, 'MMM d')}
                      </div>
                    </div>
            ...
            <div>
              ...
              <div className="space-y-1.5">
                {paidBills.map(allocation => (
                  <div
                    key={allocation.expenseId}
                    className="p-2.5 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md"
                  >
                    <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="mt-1.5 space-y-0.5">
                      ...
                      {allocation.paychecks.map((paycheck, index) => (
                        <div
                          key={paycheck.id}
                          className="text-xs text-gray-500 dark:text-gray-500 ml-3"
                        >
                          • ${allocation.amounts[index].toFixed(2)} from {paycheck.description || 'Paycheck'} 
                          ({format(paycheck.date, 'MMM d')})
                        </div>
                      ))}
                    </div>
                  </div>
